...
- `void listResources(...)`  
  - [x] Ensure the game sees the transformed paths so pack overlays work correctly. `LegacyPackResources.listResources` wraps the original list and applies the same folder + filename mapping (and rewrites listed model/blockstate JSON).
